...
class TypelyTray:
    def __init__(
        ...
        on_set_groq_api_key: Callable[[str], None],
        on_clear_groq_api_key: Callable[[], None],
        ...
        self.on_set_groq_api_key = on_set_groq_api_key
        self.on_clear_groq_api_key = on_clear_groq_api_key
    ...
    def _build_provider_menu(self) -> None:
        submenu = self.menu.addMenu("Transcription Provider")
        group = QActionGroup(submenu)
        group.setExclusive(True)

        self.provider_actions: dict[str, QAction] = {}
        for key, label in (("local", "Local Whisper"), ("groq", "Groq API")):
            action = QAction(label, submenu)
            action.setCheckable(True)
            action.setChecked(self.config.transcription_provider == key)
            action.triggered.connect(lambda checked, p=key: checked and self.on_provider_change(p))
            group.addAction(action)
            submenu.addAction(action)
            self.provider_actions[key] = action

    def _build_groq_menu(self) -> None:
        submenu = self.menu.addMenu("Groq Settings")

        set_key_action = QAction("Set API Key...", submenu)
        set_key_action.triggered.connect(self._prompt_groq_api_key)
        submenu.addAction(set_key_action)

        clear_key_action = QAction("Clear API Key", submenu)
        clear_key_action.triggered.connect(self.on_clear_groq_api_key)
        submenu.addAction(clear_key_action)

        submenu.addSeparator()

        model_submenu = submenu.addMenu("Groq Model")
        group = QActionGroup(model_submenu)
        group.setExclusive(True)
        self.groq_model_actions: dict[str, QAction] = {}
        for model_name in ("whisper-large-v3-turbo", "whisper-large-v3", "distil-whisper-large-v3-en"):
            action = QAction(model_name, model_submenu)
            action.setCheckable(True)
            action.setChecked(self.config.groq_model == model_name)
            action.triggered.connect(lambda checked, m=model_name: checked and self.on_groq_model_change(m))
            group.addAction(action)
            model_submenu.addAction(action)
            self.groq_model_actions[model_name] = action

    def _prompt_groq_api_key(self) -> None:
        value, accepted = QInputDialog.getText(
            None,
            "Groq API Key",
            "Enter Groq API key:",
        ...
        if accepted and value.strip():
            self.on_set_groq_api_key(value.strip())
