...
class TypelyController(QObject):
    ...
    def __init__(self, app: QApplication) -> None:
        ...
        self.tray = TypelyTray(
            ...
            on_provider_change=self.set_transcription_provider,
            ...
            on_set_groq_api_key=self.set_groq_api_key,
            on_clear_groq_api_key=self.clear_groq_api_key,
    ...
    def start(self) -> None:
        LOGGER.info("Typely starting: record_mode=%s provider=%s paste_mode=%s", self.config.record_mode, self.config.transcription_provider, self.config.paste_mode)
        self.hotkeys.start()
    ...
    def _transcribe_worker(self, audio_path: Path, reason: str) -> None:
        try:
            if self.config.transcription_provider == "groq":
                ...
                result = groq.transcribe(audio_path, language=None)
            else:
                result = self.transcriber.transcribe(audio_path, self.config.default_model, language=None)
            LOGGER.info(
                "Transcription complete: provider=%s model=%s language=%s duration=%.2fs chars=%d",
                self.config.transcription_provider,
                self.config.groq_model if self.config.transcription_provider == "groq" else self.config.default_model,
                result.language,
                result.duration,
                len(result.text),
            )
    ...
    def set_transcription_provider(self, provider: str) -> None:
        if provider not in {"local", "groq"}:
            return
        self.config.transcription_provider = provider
        self._save_config()
        self.tray.notify("Typely", f"Transcription provider set to {provider}")

    def set_groq_api_key(self, api_key: str) -> None:
        self.config.groq_api_key = api_key
        self._save_config()
        self.tray.notify("Typely", "Groq API key saved")

    def clear_groq_api_key(self) -> None:
        self.config.groq_api_key = None
        self._save_config()
        self.tray.notify("Typely", "Groq API key cleared")

    def set_groq_model(self, model_name: str) -> None:
        if model_name not in SUPPORTED_GROQ_MODELS:
            return
        self.config.groq_model = model_name
        self._save_config()
        self.tray.notify("Typely", f"Groq model set to {model_name}")
